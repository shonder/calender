...
def save_to_file():
    ...
    __file__) + """" /tn "Расписание с элементами напоминания" /sc MINUTE /mo {} /ed {} /F""".format(event_delay, ed_date)
    os.system('chcp 65001')
    os.system(task)


def read_from_file():
    global start_date, calc_date, description, now_date, dirname, event_delay, ed_date
    try:
...
def on_click():
    global calc_date, description, start_date, event_delay, ed_date
    start_date = now_date
    calc_date = form.calendarWidget.selectedDate()
    description = form.plainTextEdit.toPlainText()
    event_delay = form.plainTextEdit_2.toPlainText()
    ed_date = form.dateEdit.text()
